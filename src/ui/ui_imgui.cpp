...
bool UI_ImGui_Init(ALLEGRO_DISPLAY* display)
{
    g_Display = display;
    ...
    io.RenderDrawListsFn = UI_ImGui_RenderDrawLists;        // Alternatively you can set this to NULL and call ImGui::GetDrawData() after ImGui::Render() to get the same ImDrawData pointer.
#ifdef _WIN32
...
{
    UI_ImGui_InvalidateDeviceObjects();
    ImGui::Shutdown();
}
...
bool UI_ImGui_ProcessEvent(ALLEGRO_EVENT *ev)
{
    ImGuiIO &io = ImGui::GetIO();
...
void UI_ImGui_NewFrame()
{
    if (!g_Texture)
        UI_ImGui_CreateDeviceObjects();
...
bool UI_ImGui_CreateDeviceObjects()
{
    // Build texture atlas
    ImGuiIO &io = ImGui::GetIO();
...
void UI_ImGui_InvalidateDeviceObjects()
{
    if (g_Texture)
    {
